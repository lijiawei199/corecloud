...
public interface MapApi {
    ...
    @ApiOperation(value = "", nickname = "equiplist", notes = "获取地图页设备列表 安防 600004  出入口200001    扶梯200003 直梯200004", response = Equiplist.class, tags={ "地图页数据交互", })
    @ApiResponses(value = { 
    ...
    @ApiOperation(value = "", nickname = "storelist", notes = "获取地图店铺列表", response = StoreList1.class, tags={ "地图页数据交互", })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "success", response = StoreList1.class) })
    @RequestMapping(value = "/map/storelist",
        produces = { "application/json" }, 
        method = RequestMethod.GET)
    ResponseEntity<StoreList1> storelist(@NotNull @ApiParam(value = "", required = true) @Valid @RequestParam(value = "type", required = true) Integer type,@NotNull @ApiParam(value = "", required = true) @Valid @RequestParam(value = "floor", required = true) String floor);


    @ApiOperation(value = "", nickname = "totalData", notes = "地图页数据交互", response = StoreList.class, tags={ "地图页数据交互", })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "success", response = StoreList.class) })
    @RequestMapping(value = "/map/totalData",
        produces = { "application/json" }, 
        method = RequestMethod.GET)
    ResponseEntity<StoreList> totalData(@NotNull @ApiParam(value = "", required = true) @Valid @RequestParam(value = "floor", required = true) String floor,@NotNull @ApiParam(value = "0 客流量 1 销售额 2 坪效 3 集客力", required = true) @Valid @RequestParam(value = "type", required = true) String type,@NotNull @ApiParam(value = "-1表示无区分 0 日 1月 2 年", required = true) @Valid @RequestParam(value = "time", required = true) String time);
